...
The first observation is `r instacart$product_name[1]` (product id: `r instacart$product_id[1]`) bought from the `r instacart$aisle[1]` aisle (aisle id: `r instacart$aisle_id[1]`) of the `r instacart$department[1]` department (department id: `r instacart$department_id[1]`) by the user whose id is `r instacart$user_id[1]`. The order (order id: `r instacart$order_id[1]`) was placed at `r instacart$order_hour_of_day[1]` on Thursday. The order sequence number for this user is `r instacart$order_number[1]`. This product was the first product added to cart by this user and has been ordered  before. `r instacart$days_since_prior_order[1]` days has past since the last order placed by this user. The evaluation set this order belongs in is `r instacart$eval_set[1]`. 

The ninth observation is `r instacart$product_name[9]` (product id: `r instacart$product_id[9]`) bought from the `r instacart$aisle[9]` aisle (aisle id: `r instacart$aisle_id[9]`) of the `r instacart$department[9]` department (department id: `r instacart$department_id[9]`) by the user whose id is `r instacart$user_id[9]`. The order (order id: `r instacart$order_id[9]`) was placed at `r instacart$order_hour_of_day[9]` on Saturday. The order sequence number for this user is `r instacart$order_number[9]`. This product was the first product added to cart by this user and hasn't been ordered  before. `r instacart$days_since_prior_order[9]` days has past since the last order placed by this user. The evaluation set this order belongs in is `r instacart$eval_set[9]`. 
