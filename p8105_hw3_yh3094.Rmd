...
The NY NOAA data has `r nrow(ny_noaa)` observations and `r ncol(ny_noaa)` variables. Variables for `id`, `tmax` and `tmin` are class `character`. Variables for `prcp`, `snow` and `snwd` are class `integer`. Variable for `date` is class `Date`. 
